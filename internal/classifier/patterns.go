...
func defaultPatterns() []*IntentPattern {
	return []*IntentPattern{
		...
		// ============================================================
		{
			...
			Category:    "graph",
			Subcategory: "search",
			Keywords:    []string{"graph", "knowledge", "entity", "find"},
			Regex:       regexp.MustCompile(`(?i)(graph|knowledge).*(search|find|lookup)`),
			Confidence:  0.8,
			Tier:        1,
		},
		{
			...
			Category:    "graph",
			Subcategory: "link",
			Keywords:    []string{"link", "relate", "relationship", "connect"},
			Regex:       regexp.MustCompile(`(?i)(link|relate|relationship|connect)`),
			Confidence:  0.8,
			Tier:        1,
		},
		{
			...
			Category:    "graph",
			Subcategory: "ingest",
			Keywords:    []string{"ingest", "index", "add to graph"},
			Regex:       regexp.MustCompile(`(?i)(ingest|index|add).*graph`),
			Confidence:  0.8,
			Tier:        1,
		},
		...
		},
		{
			ID:          "system_open_app",
			Category:    "system",
			Subcategory: "open_app",
			Keywords:    []string{"open", "launch", "start"},
			Regex:       regexp.MustCompile(`(?i)(open|launch|start).*(app|program|chrome|edge|notepad)`),
			Confidence:  0.8,
			Tier:        1,
		},
		{
			ID:          "system_close_app",
			Category:    "system",
			Subcategory: "close_app",
			Keywords:    []string{"close", "quit", "stop"},
			Regex:       regexp.MustCompile(`(?i)(close|quit|stop).*(app|program|chrome|edge|notepad)`),
			Confidence:  0.8,
			Tier:        1,
		},
		{
			...
			Category:    "system",
			Subcategory: "list_processes",
			Keywords:    []string{"list", "processes", "running"},
			Regex:       regexp.MustCompile(`(?i)(list|show).*(processes|running)`),
			Confidence:  0.8,
			Tier:        1,
		},
		{
			ID:          "system_clipboard_read",
			Category:    "system",
			Subcategory: "clipboard_read",
			Keywords:    []string{"clipboard", "paste"},
			Regex:       regexp.MustCompile(`(?i)(read|show).*(clipboard|paste)`),
			Confidence:  0.8,
			Tier:        1,
		},
		{
			ID:          "system_clipboard_write",
			Category:    "system",
			Subcategory: "clipboard_write",
			Keywords:    []string{"copy", "clipboard"},
			Regex:       regexp.MustCompile(`(?i)(copy|set).*(clipboard)`),
			Confidence:  0.8,
			Tier:        1,
		},
		{
			ID:          "system_open_url",
			Category:    "system",
			Subcategory: "open_url",
			Keywords:    []string{"open", "url", "website"},
			Regex:       regexp.MustCompile(`(?i)(open).*(https?://|website|url)`),
			Confidence:  0.8,
			Tier:        1,
		},
		{
		...
		{
			ID:          "system_net_download",
			Category:    "system",
			Subcategory: "net_download",
			Keywords:    []string{"download", "save"},
			Regex:       regexp.MustCompile(`(?i)(download|save).*(https?://)`),
			Confidence:  0.8,
			Tier:        1,
		},
		{
			ID:          "system_schedule_run",
			Category:    "system",
			Subcategory: "schedule_run",
			Keywords:    []string{"schedule", "run at"},
			Regex:       regexp.MustCompile(`(?i)(schedule|run at).*(\d{1,2}:\d{2})`),
			Confidence:  0.8,
			Tier:        1,
		},
